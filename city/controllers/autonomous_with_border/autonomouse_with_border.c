...
double process_white_borders(const unsigned char *image, double *left_angle, double *right_angle) {
  ...
  //printf("Detected white pixels → Left: %d | Right: %d\n", left_count, right_count);


  if (left_count > 0)
    *left_angle = ((double)left_sum / left_count / camera_width - 0.5) * camera_fov;
  else
    *left_angle = UNKNOWN;

  if (right_count > 0)
    *right_angle = ((double)right_sum / right_count / camera_width - 0.5) * camera_fov;
  else
    *right_angle = UNKNOWN;
    


  return (*left_angle != UNKNOWN || *right_angle != UNKNOWN) ? 0.0 : UNKNOWN;
}
...
int main(int argc, char **argv) {
  ...
  printf("enable_collision_avoidance: %c", enable_collision_avoidance);
  ...
  while (wbu_driver_step() != -1) {
    ...
    if (i % (int)(TIME_STEP / wb_robot_get_basic_time_step()) == 0) {
      ...
      if (autodrive && has_camera) {
        ...
        process_white_borders(camera_image, &left_angle, &right_angle);
        
        const double ASSUMED_LANE_HALF_WIDTH_ANGLE = 0.15;

        bool left_missing = (left_angle == UNKNOWN);
        bool right_missing = (right_angle == UNKNOWN);
        
        // Estimate missing side using assumed lane width
        if (left_missing && !right_missing) {
          left_angle = right_angle - 2.0 * ASSUMED_LANE_HALF_WIDTH_ANGLE;
        }
        if (!left_missing && right_missing) {
          right_angle = left_angle + 2.0 * ASSUMED_LANE_HALF_WIDTH_ANGLE;
        }
        ...
        bool borders_found = (left_angle != UNKNOWN || right_angle != UNKNOWN);
        ...
        if (!borders_found) {
          if (!line_missing) {
            ...
            if(enable_display) {
              ...
              wb_display_draw_text(display, "Borders lost, autodrive off!", 10, 160);
            }
            line_missing = true;
        ...
        bool both_found = (left_angle != UNKNOWN && right_angle != UNKNOWN);
        double center_angle = UNKNOWN;
        
        if (both_found)
          center_angle = filter_angle((left_angle + right_angle) / 2.0);
        
        if (left_missing || right_missing) {
          if (enable_display)
            wb_display_draw_text(display, "⚠ Only one border visible", 10, 170);
        }
        ...
        if (enable_collision_avoidance && obstacle_angle != UNKNOWN) {
          ...
          printf("%g", obstacle_angle);
          printf("Obstacle steer = %g", obstacle_steering);
          double lane_steering = applyPID(center_angle);
          double final_steer = 0.0;
          // if we see the line we determine the best steering angle to both avoid obstacle and follow the line
          if (left_angle != UNKNOWN || right_angle != UNKNOWN) {
            if (obstacle_steering > 0 && lane_steering > 0)
              final_steer = fmax(obstacle_steering, lane_steering);
            else if (obstacle_steering < 0 && lane_steering < 0)
              final_steer = fmin(obstacle_steering, lane_steering);
            else 
              final_steer = lane_steering;
              
          } else
            PID_need_reset = true;
          // apply the computed required angle
          set_steering_angle(final_steer);
        } else if (obstacle_angle == UNKNOWN && (left_angle != UNKNOWN || right_angle != UNKNOWN)) {
          // no obstacle has been detected, simply follow the line
          wbu_driver_set_brake_intensity(0.0);
          set_steering_angle(applyPID(center_angle));
        } else {
